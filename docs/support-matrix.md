...
| Production portal identity and restored binding | Not tested | The canonical desktop and user-service assets pass headless package verification but are not installed on PC1. Registry registration, cgroup inference, and restored binding remain untested in the production runtime, so prototype restoration cannot qualify the identity. |
