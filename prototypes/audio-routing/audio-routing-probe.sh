...
capture_state() {
  ...
  wpctl status >"$out_dir/wpctl-status.txt" 2>&1 || true
  pactl -f json list sinks >"$out_dir/pulse-sinks.json"
  pactl -f json list sources >"$out_dir/pulse-sources.json"
  pactl -f json list sink-inputs >"$out_dir/pulse-sink-inputs.json"
  pactl -f json list source-outputs >"$out_dir/pulse-source-outputs.json"
  pw-dump >"$out_dir/pipewire-dump.json"
}
...
set +e
pw-record --target "$sink_name.monitor" --rate 48000 --channels 2 --format s16 --sample-count 57600 "$capture" >"$out_dir/pw-record.log" 2>&1 &
record_pid=$!
sleep 0.15
...
if [[ "$play_status" -eq 0 && "$record_status" -eq 0 && -s "$capture" ]]; then
...
jq -n \
  --slurpfile sinks "$out_dir/pulse-sinks.json" \
  --slurpfile sources "$out_dir/pulse-sources.json" \
  --slurpfile inputs "$out_dir/pulse-sink-inputs.json" \
  --slurpfile outputs "$out_dir/pulse-source-outputs.json" \
    ...
    else "unclassified" end;
  {
    ...
    synthetic_capture: {status: $synthetic_status, sink: $sink_name, source: ($sink_name + ".monitor"), file: "synthetic-monitor.wav", verification: "synthetic-ffprobe.json and synthetic-astats.txt"},
    ...
    sinks: [$sinks[0][] | {description, persistence}],
    non_monitor_sources: [$sources[0][] | select((.name | endswith(".monitor")) | not) | {description, persistence}],
    output_streams: [$inputs[0][] | {index, sink, classification: classify, application: {name: properties["application.name"], binary: properties["application.process.binary"], media_name: properties["media.name"]}, persistence}],
    input_streams: [$outputs[0][] | {index, source, classification: classify, application: {name: properties["application.name"], binary: properties["application.process.binary"], media_name: properties["media.name"]}, persistence}],
    raw_state: ["pactl-info.txt", "wpctl-status.txt", "pulse-sinks.json", "pulse-sources.json", "pulse-sink-inputs.json", "pulse-source-outputs.json", "pipewire-dump.json"]
  }' >"$out_dir/report.json"
