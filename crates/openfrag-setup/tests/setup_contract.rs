...
fn demo_import_remains_ready_when_capture_is_unavailable() {
    ...
    assert_eq!(
        report.check(CheckId::DemoImport).unwrap().status,
        CheckStatus::Ready
    );
    assert_eq!(
        report.check(CheckId::Capture).unwrap().status,
        CheckStatus::Blocked
    );
    assert_eq!(
        report.check(CheckId::ManualFlag).unwrap().status,
        CheckStatus::Blocked
    );
    assert!(
        report
            .check(CheckId::Capture)
            .unwrap()
            .action
...
fn capture_requires_the_recorder_but_audio_is_an_actionable_warning() {
    let report = evaluate(&HostFacts {
        data_directory_writable: true,
        cs2_cfg_directory: Some("/steam/cs2/cfg".into()),
        recorder: Some(RecorderInstall::Native(
            "/usr/bin/gpu-screen-recorder".into(),
        )),
    ...
    assert_eq!(
        report.check(CheckId::GsiConfig).unwrap().status,
        CheckStatus::Ready
    );
    assert_eq!(
        report.check(CheckId::ManualFlag).unwrap().status,
