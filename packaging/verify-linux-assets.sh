...
require_line "$service" 'ExecStart=%h/.local/bin/openfragd serve --bind 127.0.0.1:9680'
...
remote_endpoints=$(grep -Eho 'https?://[^[:space:]]+' "$service" "$desktop" "$helper" | grep -Fv 'http://127.0.0.1:9680/' | grep -Fv 'http://${host}:${port}/' || true)
